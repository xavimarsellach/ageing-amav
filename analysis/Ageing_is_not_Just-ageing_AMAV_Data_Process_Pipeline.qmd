---
title: "*Ageing is not just ageing* and *Rising Disease Prevalence Signals Epigenetic Degeneration in Humans* — Data Analysis Pipeline"
...
# Required output file (created from AMAV_DATA.xlsx)
nm_path   <- file.path("output", "AMAV_DATA-no-monotonic_AUTO.xlsx")

# The correct script that produces the “no-monotonic” dataset:
# it trims deterministic monotonic prefixes from FOLD_RELATIVE
nm_script <- file.path("scripts", "remove_monotonic_prefix_FOLD_RELATIVE.R")

# Ensure "no-monotonic" file exists
if (!file.exists(nm_path)) {
  if (file.exists(nm_script)) {
    cat("ℹ️ 'AMAV_DATA-no-monotonic_AUTO.xlsx' not found; running remove_monotonic_prefix_FOLD_RELATIVE.R...\n")
    source(nm_script)
  } else {
    stop("Missing both the no-monotonic file AND the R script that generates it. Cannot continue.")
  }
}

# Double-check that the file was created
if (!file.exists(nm_path)) {
  stop("AMAV_DATA-no-monotonic_AUTO.xlsx is still missing after running remove_monotonic_prefix_FOLD_RELATIVE.R.")
}
...
if (file.exists(fig_script)) {
  ...
  cat("⚠️ FIGURE_S1B.R not found; skipping Supplemental Figure S1B.\n")
}
```
...
if (file.exists(sup_script)) {
  ...
  cat("⚠️ make_supplemental_figures.py not found; skipping Python supplemental figures.\n")
}
```
